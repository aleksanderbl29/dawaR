...
First we need to load the packages. Here, we only load `{ggplot2}` to minimise dependencies, but you can definately load the entire `{tidyverse}` for convenience.
...
```{r print_map_municipality, eval=FALSE}
...
obj <- ggplot(municipalities, aes(fill = navn)) +
  geom_sf() +
  cowplot::theme_map() +
  theme(legend.position = "none")
saveRDS(obj, "vignettes/muni_obj_graph.rds")
```

```{r, echo=FALSE}
readRDS("vignettes/muni_obj_graph.rds")
```
