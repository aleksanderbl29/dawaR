...
According to [this issue on GitHub](https://github.com/rstudio/renv/issues/1832) the issue is caused by `renv` installing the version of `{sf}` from "https://packagemanager.posit.co/cran/latest" where the `gdal` subdirectory is not included in the `{sf}` package from this repo - but it is included in the version of `{sf}` from "https://cran.rstudio.com/". To work around this you should set the CRAN repo before installing the package.
